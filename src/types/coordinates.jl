...
abstract type FCurv <: CoordFrame end
struct FRoute <: FCurv end 
struct FLanelet <: FCurv end
...
"""
struct Pos{F<:Union{FCart, FRoute, FLanelet}} <: FieldVector{2, Float64}
    c1::Float64 # x / east in case of FCart; s in case of FCurv
    c2::Float64 # y / north in case of Fcart; d in case of FCurv
end

Pos(::Type{F}, c1::T1, c2::T2) where {F<:Union{FCart, FRoute, FLanelet}, T1<:Number, T2<:Number} = Pos{F}(c1, c2)
Pos(::Type{F}, sv::SVector{2, T}) where {F<:Union{FCart, FRoute, FLanelet}, T<:Number} = Pos{F}(sv[1], sv[2])
...
"""
struct Vec{F<:Union{FCart, FRoute, FLanelet}} <: FieldVector{2, Float64}
    Δc1::Float64 # Δx in case of FCart; Δs in case of FCurv
    Δc2::Float64 # Δy in case of FCart; Δd in case of FCurv
end

Vec(::Type{F}, Δc1::T1, Δc2::T2) where {F<:Union{FCart, FRoute, FLanelet}, T1<:Number, T2<:Number} = Vec{F}(Δc1, Δc2)
Vec(::Type{F}, sv::SVector{2, T}) where {F<:Union{FCart, FRoute, FLanelet}, T<:Number} = Vec{F}(sv[1], sv[2])
...
struct TransFrame{F}
    ...
    function TransFrame(::Type{F}, ref_pos::Vector{Pos{FCart}}) where {F<:Union{FRoute, FLanelet}}
        @assert length(ref_pos) ≥ 2
        cum_dst = cumsum(norm.(diff(ref_pos)))
        pushfirst!(cum_dst, 0.0)
        return new{F}(ref_pos, cum_dst)
    end

    function TransFrame(::Type{F}) where {F<:Union{FRoute, FLanelet}}
        return new{F}(Vector{Pos{FCart}}(), Vector{Float64}())
    end
end

function transform(pos::Pos{F}, frame::TransFrame{F}) where {F<:Union{FRoute, FLanelet}}
...
function transform(::Type{F}, pos::Pos{FCart}, frame::TransFrame{F}) where {F<:Union{FRoute, FLanelet}}
    ...
    # choose smallest distance and return Pos{F}
